...
└─ src/
   ...
   ├─ strategy_ref.py       # TQQQ·QLD 공격형 전략 정리 팝업(비중·분산기간·주기 롤링 결론, 공유용 HTML)
   ├─ whatif.py             # 상단 대시보드 '1천만원 N년 전 매수 시' TQQQ·QQQ·QLD 실측 수익률(오늘 기준 자동갱신)
...
| 1.12.0 | 참고자료에 '🚀 TQQQ·QLD 공격형 전략 정리' 팝업(strategy_ref.py) 추가 — 대화로 검증한 롤링 백테스트 결론을 한 장 정리: ①비중=레버리지 다이얼(상관0.999, 2.5~2.75배 권장, 표), ②분산기간 6개월~1년·주기 무차별(매월 권장), ③거치식/2년 비권장, 최종결론+위험경고, 공유용 HTML 다운로드. 기반: 2010~ 롤링 3년보유·월리밸·배당재투자TR·세금/환율/수수료 제외 |
...
- `C:\python\백테스트GUI_배포판.zip` (약 190KB, Python 필요, 1_설치.bat 방식) — **v1.13.4 최신화 완료(2026-07-13)**
- `C:\python\백테스트GUI_포터블판.zip` (약 116MB, 무설치, 임베디드 Python 내장) — **v1.13.4 최신화 완료(2026-07-13)**
- `C:\python\백테스트GUI_포터블` (포터블 원본 폴더, 재압축 소스)
- `C:\python\backtest_gui_완성본 1`, `backtest_gui_완성본 2` (백업 스냅샷)
- ✅ **두 배포물 모두 최신 소스 반영됨**(게시판·중국홍콩·전략정리·상단 대시보드 등 포함, 새 기능은 추가 패키지 불필요).
- **재패키징 방법**: ①배포판=레포의 top파일+.streamlit+config+data/reference_metadata.json+src/(.py·.csv)를 `백테스트GUI/` 루트로 zip. ②포터블=`백테스트GUI_포터블/src/`의 .py·csv와 data/reference_metadata.json만 레포 최신으로 덮고(main.py·requirements·python 임베디드는 포터블 전용이라 유지) 폴더 전체를 `백테스트GUI_포터블/` 루트로 재압축(__pycache__ 제외). BAT 파일은 이미 CP949+CRLF라 바이트 복사로 인코딩 유지.
...
### 11-6. 현재 상태 요약 (2026-07-13 기준)
- 최신 버전 **v1.13.4**, 모든 커밋 push 완료(작업트리 깨끗·origin 동기화), 웹앱 정상.
- 4개 모드: 📈 가격 백테스트 / 🎯 최적의 투자 루틴 추천 / 💵 적립식 현금관리 계산기 / 💬 의견 게시판.
- 최근 작업: 의견 게시판(구글폼), 중국·홍콩 자산/총정리, TQQQ·QLD 공격형 전략 정리, 상단 '1천만원 N년 전' 대시보드(거치/적립/라오어V3).
- **로컬 배포물(배포판·포터블판) 2026-07-13 v1.13.4로 재패키징 완료**(§7), 백업 완성본 2 생성.
- 바로 이어서 할 만한 것: §9 로드맵 (진입시점 민감도, 추세필터 등).
