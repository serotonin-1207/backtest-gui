...
24개 자동 테스트: CAGR·MDD·XIRR·TWR 수학적 정답 대조, 거치식=가격비율 폐형식, 수수료 폐형식, 적립식 투입합, 인출 반영, 라오어 V2.2/V3.0 불변식(현금≥0, equity>0, T∈[0,40]), 현금관리 스펙 검증값, 환율 왕복, 세금 공제 후 22% 정답, 국내주식 비과세, 합성 상관>0.99 — **전부 통과**.
발견·수정된 버그: ①적립식 총수익률(v2), ②무회복일 고점일 기준(1.5.0). 알려진 근사(버그 아님): 세금 만기청산 일괄, 라오어 체결 낙관(수수료로 보정 가능), 배당 TR은 연율 근사, 표시 환산은 현재 환율(엔진 계산은 일별 FX 지원).
